...
export function resolveAliases() {
    test.equal(QueryBuilder.resolveAliases(new SqlExpression("{this}.name"), "user", "user", "user").sql, '"user".name', "resolveSimple");          
    test.equal(QueryBuilder.resolveAliases(new SqlExpression("{author}.name"), "user", "").sql, '"author".name', "resolveSimple");         
}
