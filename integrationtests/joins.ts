...
class User {
	...
	@joinOne(PersonalDetails)
	personalDetails: PersonalDetails;
}

export function testJoins(): Promise<void> {
    return testdb.withEmptyDatabase(async (db) => {
        await db.query("CREATE TABLE `user` (user_id INT NOT NULL AUTO_INCREMENT, username VARCHAR(255), personaldetails_id BIGINT NOT NULL, PRIMARY KEY(user_id))");
        ...
        await db.query(sql`INSERT INTO user (username, personaldetails_id) VALUES ('john', ${personalDetailsId})`);
        
        let users = await new QueryBuilder(User).execute<User>(db);
        test.equal(users.length, 1);

        await db.query(sql`UPDATE user SET personaldetails_id=NULL`);
